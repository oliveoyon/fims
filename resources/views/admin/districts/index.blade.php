        ...
        <div class="row mb-3">
            <div class="col">
                <button class="btn btn-success btn-sm" id="createDistrictBtn">
                    <i class="fas fa-plus-square mr-1"></i> Add New District
                </button>
            </div>
        </div>
            ...
            <tbody>
                @foreach ($districts as $district)
                    <tr id="district-{{ $district->id }}">
                        ...
                        <td class="district-zone">{{ $district->zone->name ?? '-' }}</td>
                        <td>
                            <button class="btn btn-warning btn-sm editDistrictBtn"
                                    data-id="{{ $district->id }}"
                                    data-name="{{ $district->name }}"
                                    data-zone="{{ $district->zone_id }}">
                                Edit
                            </button>
                ...
                <div class="modal-body">
                    <form id="districtForm" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="zoneSelect" name="zone_id">
...
<script>
    ...
    document.getElementById('createDistrictBtn').addEventListener('click', function() {
        ...
        document.getElementById('districtForm').setAttribute('action', '{{ route('admin.districts.store') }}');
    ...
    document.querySelectorAll('.editDistrictBtn').forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            document.getElementById('districtForm').setAttribute('action',
                '{{ route('admin.districts.update', ':id') }}'.replace(':id', districtId));
            document.getElementById('districtForm').setAttribute('method', 'POST');
        ...
        button.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch('{{ route('admin.districts.destroy', ':id') }}'.replace(':id', districtId), {
                        method: 'DELETE',
                        headers: {
    ...
    document.getElementById('districtForm').addEventListener('submit', function(event) {
        ...
        .then(data => {
            if (data.success) {
                if (districtModal) districtModal.hide();
